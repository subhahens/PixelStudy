  ...
  Math: [
    {
      ...
      desc: "Free lessons & practice problems for many math topics",
    },
    {
      name: "Desmos",
      url: "https://www.desmos.com",
      desc: "Graphing calculator and activities",
    },
  ],
  "Computer Science": [
    {
      ...
      desc: "Documentation and guides for web dev",
    },
    {
      name: "freeCodeCamp",
      url: "https://www.freecodecamp.org",
      desc: "Free interactive coding lessons",
    },
  ],
  Biology: [
    {
      ...
      desc: "Engaging video explanations",
    },
  ],
};
...
function renderSubjects() {
  ...
  if (keys.length === 0) {
    subjectsEl.innerHTML =
      '<p style="opacity:0.7">No subjects yet — add one to get started</p>';
    return;
  }
  keys.forEach((s) => {
    ...
    el.innerHTML = `<div style="display:flex;gap:0.65rem;align-items:center"><div class="tag">${
      s[0] || "?"
    }</div><div class="meta">${s}</div></div><div class="count">${
      resources[s].length
    }</div>`;
...
function renderGrid(filterText = "") {
  ...
  const pairs = [];
  if (currentSubject) {
    (resources[currentSubject] || []).forEach((r, i) =>
      pairs.push([currentSubject, r, i])
    );
  } else {
    ...
    btn.addEventListener("click", (e) => {
      ...
      openModal(() => {
        ...
        resources[ns].push({
          name: nn,
          url: nu,
          desc: resources[s] && resources[s][idx] ? resources[s][idx].desc : "",
        });
        saveResources(resources);
        renderAll();
        return true;
...
function onSaveDefault() {
  ...
  if (nn.length < 4) {
    alert("Resource name must be at least 4 characters.");
    return false;
  }
  const urlPattern = /^(https?:\/\/)[\w.-]+(\.[\w.-]+)+[/\w\-.?=&%]*/;
  if (!urlPattern.test(nu)) {
...
document.getElementById("reset-sample").addEventListener("click", () => {
  if (
    confirm("Reset to sample data? This will overwrite your saved resources.")
  ) {
